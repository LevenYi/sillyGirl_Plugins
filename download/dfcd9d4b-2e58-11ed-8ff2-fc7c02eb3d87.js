...
//	Notify+="开始调整青龙变量顺序\n"
	for(let i=0;i<QLS.length;i++){
		...
		let ql_token=Get_QL_Token(QLS[i].host,QLS[i].client_id,QLS[i].client_secret)
		if(ql_token==null){
			Notify+="\n容器获取token失败，跳过\n"
			continue
		}
//		s.reply(JSON.stringify(ql_token))
		let envs=Get_QL_Envs(QLS[i].host,ql_token)
		...
		if(SelectMode==1){	//欧气模式
			while(move.length<MoveNum){
				let index=RandomExcept(0,envs.length,NotMove[i])
				if(!FindEle(move,index)&&envs[index].status!=1)//防止随机到同一个变量
					move.push(index)				
			}
			...
			for(let j=envs.length-1;j>=0;j--){
				if(!FindEle(NotMove[i],j+1)&&envs[j].status!=1)
					move.push(j)
