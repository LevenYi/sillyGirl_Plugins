 ...
 * @description 项目地址教程https://github.com/pengzhile/pandora/blob/master/doc/wiki.md
 * @title pandora
 * @rule ai ? ?
 * @rule ai ?
 * @rule ai list
...
/*
pandora token更新：https://gist.github.com/pengzhile/448bfcfd548b3ae4e665a84cc86c4694，定时运行脚本，token自动保存至share_tokens.txt
...
function main(){
    ...
    if(s.param(2)){ //指定对话
        ...
        prompt=s.param(1)
        if(DefaultCon&&conversations.items.length){
    ...
    let conversation_id=cvs? cvs.id: undefined // 首次对话可不传。ChatGPT回复时可获取。
    let parent_message_id=conversation_id? Req("/api/conversation/"+conversation_id).current_node:uuid()   // 父消息ID，首次同样需要生成。之后获取上一条回复的消息ID即可。
